...
/*
    ...
    {
        if(Configuration.Instance.Cache.ReuseHtml && MemoryStore.ClientPageHtml != null) return MemoryStore.ClientPageHtml;
        ...
        if (Configuration.Instance.Client.AddPrefetchTags)
            html = html.Replace("<!--prefetch_script-->",
                string.Join("\n", MemoryStore.PreloadScripts.Select(x => $"<link rel=\"prefetch\" as=\"script\" href=\"{x}\" />")));
        html = html.Replace("<!--client_script-->",
            string.Join("\n", MemoryStore.Scripts.Select(x => $"<script src=\"{x}\"></script>")));
        html = html.Replace("<!--client_css-->",
            string.Join("\n", MemoryStore.Stylesheets.Select(x => $"<link rel=\"stylesheet\" href=\"{x}\" />")));
        ...
        if (debugOptions.DumpWebsocketTraffic)
            html = html.Replace("<!-- preload plugin marker -->",
                ...
                "\n<!-- preload plugin marker -->");
        html += (await File.ReadAllTextAsync(RuntimeEnvironment.BinDir + "/Resources/Private/WelcomeScreenWrapper.html"))
            .Replace("<!-- content -->", await File.ReadAllTextAsync(RuntimeEnvironment.BinDir + "/Resources/Overridable/WelcomeScreen/index.html"));
        MemoryStore.ClientPageHtml = html;
        return html;
    }
